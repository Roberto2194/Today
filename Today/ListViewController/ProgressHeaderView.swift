...
// UICollectionReusableView is used to create supplementary views.
// Supplementary views are separate from the individual collection
// view cells, so they are ideal for creating headers or footers.
class ProgressHeaderView: UICollectionReusableView {
    ...
    var progress: CGFloat = 0 {
        didSet {
            // To ensure that the progress state and the view that represents it are always in sync,
            // we’ll update the progress view every time the progress changes.
            setNeedsLayout()
